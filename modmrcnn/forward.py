...
def my_forward(self, images, targets=None):
...
def fake_proposals(masks):
    ...
    # objectness = torch.stack([torch.mean(mask[0, bbox[0]:bbox[2], bbox[1]:bbox[3]]) if bbox is not None else torch.tensor(0.).to(mask.device) for bbox, mask in zip(bboxes, masks)]) # 
    objectness = torch.stack([torch.mean(mask[0]) if bbox is not None else torch.tensor(0.).to(mask.device) for bbox, mask in zip(bboxes, masks)]) # 
    # objectness = torch.stack([torch.tensor(1.) if bbox is not None else torch.tensor(0.).to(mask.device) for bbox, mask in zip(bboxes, masks)]) # 
